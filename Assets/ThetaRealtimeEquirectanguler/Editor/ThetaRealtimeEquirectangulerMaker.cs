...
public class ThetaRealtimeEquirectangulerMaker
{
	...
	public static void InitBoth()
	{
		...
		if( !File.Exists( PrefabAssetPath_Both ) ) {
			...
			go = AssetDatabase.LoadAssetAtPath(PrefabAssetPath_Both, typeof(GameObject)) as GameObject;
			...
			meshRenderer.sharedMaterial = AssetDatabase.LoadAssetAtPath(MaterialAssetPath_Both, typeof(Material)) as Material;
	...
	public static void InitForwardBackward()
	{
		...
		if( !File.Exists( PrefabAssetPath_ForwardBackward ) ) {
...

			{
				...
				meshRenderer.sharedMaterial = AssetDatabase.LoadAssetAtPath(MaterialAssetPath_Forward, typeof(Material)) as Material;
			}
			{
				...
				meshRenderer.sharedMaterial = AssetDatabase.LoadAssetAtPath(MaterialAssetPath_Backward, typeof(Material)) as Material;
			}
